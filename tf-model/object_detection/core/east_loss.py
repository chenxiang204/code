...
class WeightedIOULocalizationLoss(Loss):
  ...
  def _compute_loss(self, prediction_tensor, target_tensor, angle_weights, weights):
    """Compute loss function.
    ...
    per_anchor_iou_loss = -1.0 * tf.log(box_list_ops.matched_iou(predicted_boxes,
                                                         target_boxes))
    angle_loss = angle_weights * (1.0 - tf.cos(tf.sub(prediction_tensor[:,:,4], target_tensor[:,:,4])))
    return tf.reduce_sum(tf.reshape(weights, [-1]) * per_anchor_iou_loss) + tf.reduce_sum(angle_loss) 
...
class WeightedEastMaskLoss(Loss):
  ...
  def _compute_loss(self, prediction_tensor, target_tensor, weights, fore_weights):
    """Compute loss function.
    ...
    Returns:
      loss: a (scalar) tensor representing the value of the loss function
    """
    loss1 = -1.0 * fore_weights * tf.multiply(target_tensor, 
                                                       tf.log(prediction_tensor))
    loss2 = -1.0*(1-fore_weights)*tf.multiply(tf.log(1-target_tensor)), (1-target_tensor)

    return tf.reduce_sum(tf.add(loss1, loss2))
