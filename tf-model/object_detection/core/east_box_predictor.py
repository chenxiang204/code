  ...
  def _predict(self, image_features, num_predictions_per_location):
    ...
    with slim.arg_scope(self._conv_hyperparams), \
      ...
      with slim.arg_scope([slim.conv2d], activation_fn=None,
        ...
        mask_encodings = slim.conv2d(
            net, num_predictions_per_location * num_class_slots,
            [self._kernel_size, self._kernel_size], scope='MaskPredictor')
        if self._apply_sigmoid_to_scores:
          mask_encodings = tf.sigmoid(
              mask_encodings)
    ...
    if batch_size is None:
      ...
      mask_encodings = tf.reshape(
          mask_encodings,
          [-1, flattened_predictions_size, 1, num_class_slots])
    else:
      box_encodings = tf.reshape(
          box_encodings, [batch_size, -1, 1, self._box_code_size])
      mask_encodings = tf.reshape(
          mask_encodings, [batch_size, -1, 1, num_class_slots])
    return {BOX_ENCODINGS: box_encodings,
            MASK_PREDICTIONS:
            mask_encodings}
