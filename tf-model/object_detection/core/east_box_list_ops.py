...
def east_matched_intersection(boxlist1, boxlist2, scope=None):
  ...
  """
  with tf.name_scope(scope, 'EastMatchedIntersection'):
    y_min1, x_min1, y_max1, x_max1 = tf.split(
        value=boxlist1.get(), num_or_size_splits=4, axis=1)
    y_min2, x_min2, y_max2, x_max2 = tf.split(
        value=boxlist2.get(), num_or_size_splits=4, axis=1)
...
def east_matched_iou(boxlist1, boxlist2, scope=None):
  ...
  """
  with tf.name_scope(scope, 'EastMatchedIOU'):
    intersections = east_matched_intersection(boxlist1, boxlist2)
    areas1 = area(boxlist1)
    areas2 = area(boxlist2)
    unions = areas2
    return tf.where(
