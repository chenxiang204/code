...
from object_detection.core import east_box_coder as bcoder
from object_detection.core import east_box_list
...
class EASTMetaArch(model.DetectionModel):
  ...
  def predict(self, preprocessed_inputs):
    ...
    (box_encodings, angle_encodings, class_predictions_with_background
  ...
  def loss(self, prediction_dict, scope=None):
    ...
    with tf.name_scope(scope, 'Loss', prediction_dict.values()):
      ...
      # by chenx
      mask_concat = tf.concat([self._mask, self._mask, self._mask, 
                               self._mask, self._mask], axis=2)
      prediction_dict['box_encodings'] = tf.multiply(
                        prediction_dict['box_encodings'], mask_concat)
      fore_num = tf.reduce_sum(self._mask)
      fore_weights = 1.0 - fore_num / (tf.shape(prediction_dict)[1]*tf.shape(prediction_dict)[2])
      ##################################################################
      ...
      cls_losses = self._classification_loss(
          prediction_dict['mask_encodings'],
          batch_cls_targets,
          fore_weights,
      ...
      normalizer_loc = tf.constant(1.0, dtype=tf.float32)
      normalizer_mask = tf.constant(tf.shape(prediction_dict)[1], dtype=tf.float32)
      if self._normalize_loss_by_num_matches:
        normalizer_loc = tf.maximum(tf.to_float(tf.reduce_sum(fore_num)), 1.0)

      loss_dict = {
          'localization_loss': (self._localization_loss_weight / normalizer_loc) *
                               localization_loss,
          'classification_loss': (self._classification_loss_weight /
                                  normalizer_mask) * classification_loss
      }
    return loss_dict
